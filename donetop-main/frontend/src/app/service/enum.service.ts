...
export class EnumService {
  ...
  async categoryArray() {
    ...
    this._categoryArray = (await firstValueFrom(this.httpClient.get<Response<Category[]>>('/api/enum/category'))).data;
    return this._categoryArray;
  }

  async paymentMethodArray() {
    if (this._paymentMethodArray !== undefined) return this._paymentMethodArray;
    this._paymentMethodArray = (await firstValueFrom(this.httpClient.get<Response<Enum[]>>('/api/enum/paymentMethod'))).data;
