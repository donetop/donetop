:doctype:
:docinfo: shared
:toc: left
:toclevels: 3

= Draft API Documentation

== Draft Single Create

Http Method : *POST* +
Uri : */api/draft*

=== Request parts description
include::{snippets}/draft_single_create/createSingle_withValidParamsAndFiles_return200/request-parts.adoc[]
=== Response body fields description
include::{snippets}/draft_single_create/createSingle_withValidParamsAndFiles_return200/response-fields.adoc[]

=== Examples
==== createSingle_withoutParams_return400
.request
include::{snippets}/draft_single_create/createSingle_withoutParams_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_create/createSingle_withoutParams_return400/http-response.adoc[]

==== createSingle_withInvalidParams_return400
.request
include::{snippets}/draft_single_create/createSingle_withInvalidParams_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_create/createSingle_withInvalidParams_return400/http-response.adoc[]

==== createSingle_withValidParamsAndWithoutFiles_return200
.request
include::{snippets}/draft_single_create/createSingle_withValidParamsAndWithoutFiles_return200/http-request.adoc[]
.response
include::{snippets}/draft_single_create/createSingle_withValidParamsAndWithoutFiles_return200/http-response.adoc[]

==== createSingle_withValidParamsAndSizeExceedFiles_return400
.request
include::{snippets}/draft_single_create/createSingle_withValidParamsAndSizeExceedFiles_return400/httpie-request.adoc[]
.response
include::{snippets}/draft_single_create/createSingle_withValidParamsAndSizeExceedFiles_return400/http-response.adoc[]

==== createSingle_withValidParamsAndFiles_return200
.request
include::{snippets}/draft_single_create/createSingle_withValidParamsAndFiles_return200/httpie-request.adoc[]
.response
include::{snippets}/draft_single_create/createSingle_withValidParamsAndFiles_return200/http-response.adoc[]

== Draft Single Update

Http Method : *PUT* +
Uri : */api/draft/\{draft_id\}*

=== Request parts description
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndFilesAndId_return200/request-parts.adoc[]

=== Response body fields description
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndFilesAndId_return200/response-fields.adoc[]

=== Examples
==== updateSingle_withInvalidParams_return400
.request
include::{snippets}/draft_single_update/updateSingle_withInvalidParams_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_update/updateSingle_withInvalidParams_return400/http-response.adoc[]

==== updateSingle_withValidParamsButUnknownId_return400
.request
include::{snippets}/draft_single_update/updateSingle_withValidParamsButUnknownId_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_update/updateSingle_withValidParamsButUnknownId_return400/http-response.adoc[]

==== updateSingle_withValidParamsAndId_return200
.request
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndId_return200/http-request.adoc[]
.response
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndId_return200/http-response.adoc[]

==== updateSingle_withValidParamsAndFilesAndId_return200
.request
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndFilesAndId_return200/httpie-request.adoc[]
.response
include::{snippets}/draft_single_update/updateSingle_withValidParamsAndFilesAndId_return200/http-response.adoc[]

== Draft Single Get

Http Method : *GET* +
Uri : */api/draft/\{draft_id\}*?password=\{string\}

=== Request parameters description
include::{snippets}/draft_single_get/getSingleThatHasFolder_withValidIdAndRightPassword_return200/request-parameters.adoc[]

=== Response body fields description
include::{snippets}/draft_single_get/getSingleThatHasFolder_withValidIdAndRightPassword_return200/response-fields.adoc[]

=== Examples
==== getSingle_withInvalidId_return400
.request
include::{snippets}/draft_single_get/getSingle_withInvalidId_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_get/getSingle_withInvalidId_return400/http-response.adoc[]

==== getSingle_withValidIdButWrongPassword_return400
.request
include::{snippets}/draft_single_get/getSingle_withValidIdButWrongPassword_return400/http-request.adoc[]
.response
include::{snippets}/draft_single_get/getSingle_withValidIdButWrongPassword_return400/http-response.adoc[]

==== getSingle_withValidIdAndRightPassword_return200
.request
include::{snippets}/draft_single_get/getSingle_withValidIdAndRightPassword_return200/http-request.adoc[]
.response
include::{snippets}/draft_single_get/getSingle_withValidIdAndRightPassword_return200/http-response.adoc[]

==== getSingleThatHasFolder_withValidIdAndRightPassword_return200
.request
include::{snippets}/draft_single_get/getSingleThatHasFolder_withValidIdAndRightPassword_return200/http-request.adoc[]
.response
include::{snippets}/draft_single_get/getSingleThatHasFolder_withValidIdAndRightPassword_return200/http-response.adoc[]

== Draft Multiple Get

Http Method : *GET* +
Uri : */api/drafts?page=\{int\}&size=\{int\}&property=\{draft_property\}&direction=\{asc_or_desc\}*

=== Request parameters description
include::{snippets}/draft_multiple_get/getMultiple_withValidParams_return200/request-parameters.adoc[]

=== Response body fields description
include::{snippets}/draft_multiple_get/getMultiple_withValidParams_return200/response-fields.adoc[]

=== Examples
==== getMultiple_withInvalidPageValue_return400
.request
include::{snippets}/draft_multiple_get/getMultiple_withInvalidPageValue_return400/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withInvalidPageValue_return400/http-response.adoc[]

==== getMultiple_withInvalidSizeValue_return400
.request
include::{snippets}/draft_multiple_get/getMultiple_withInvalidSizeValue_return400/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withInvalidSizeValue_return400/http-response.adoc[]

==== getMultiple_withInvalidDirectionValue_return400
.request
include::{snippets}/draft_multiple_get/getMultiple_withInvalidDirectionValue_return400/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withInvalidDirectionValue_return400/http-response.adoc[]

==== getMultiple_withInvalidPropertyValue_return400
.request
include::{snippets}/draft_multiple_get/getMultiple_withInvalidPropertyValue_return400/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withInvalidPropertyValue_return400/http-response.adoc[]

==== getMultiple_withValidParams_return200
.request
include::{snippets}/draft_multiple_get/getMultiple_withValidParams_return200/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withValidParams_return200/http-response.adoc[]

==== getMultiple_withoutParams_return200
.request
include::{snippets}/draft_multiple_get/getMultiple_withoutParams_return200/http-request.adoc[]
.response
include::{snippets}/draft_multiple_get/getMultiple_withoutParams_return200/http-response.adoc[]