...
public class CustomerPostSingleDeleteTest extends CustomerPostBase {
	...
	void deleteSingle_withoutSession_return400() {
		...
		given.filter(
			document(
				"customer_post_single_delete/deleteSingle_withoutSession_return400"
			)
		);
	...
	void deleteSingle_withUnknownId_return400() throws Exception {
		...
		given.filter(
			document(
				"customer_post_single_delete/deleteSingle_withUnknownId_return400"
			)
		);
	...
	void deleteSingle_byNormalUser_return400() throws Exception {
		...
		given.filter(
			document(
				"customer_post_single_delete/deleteSingle_byNormalUser_return400"
			)
		);
		...
		final Response response = given.when()
			...
			.delete(SINGULAR + "/{id}", customerPost.getId());
	...
	void deleteSingle_byAdminUser_return200() throws Exception {
		...
		given.filter(
			document(
				"customer_post_single_delete/deleteSingle_byAdminUser_return200",
				responseFields(
					fieldWithPath("status").type(STRING).description("Status value."),
					fieldWithPath("code").type(NUMBER).description("Status code."),
					fieldWithPath("data").type(NUMBER).description("This is customer post id.")
				)
			)
		);
		...
		response.then()
			...
			.body("data", is((int) customerPost.getId()));
	}
